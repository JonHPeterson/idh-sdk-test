...
from idh_ota_client.api.metadata import get_api_v1_metadata_sources
from idh_ota_client.api.metadata import get_api_v_1_metadata_sources_name_or_uuid
from idh_ota_client.api.metadata import get_api_v_1_metadata_sources_srctags_source_uuid
from idh_ota_client.api.metadata import get_api_v_1_metadata_sources_statesets_source_uuid
from idh_ota_client.api.metadata import get_api_v_1_metadata_sources_statesets_name_source_uuid
from idh_ota_client.api.metadata import get_api_v_1_metadata_sources_statesets_id_source_uuid
...
def main():
    ...
    try:
        sources = get_api_v1_metadata_sources.sync(client=client)
        ...
        if first_source:
            source_response = get_api_v_1_metadata_sources_name_or_uuid.sync(client=client, name_or_uuid=first_source)
            print(f"SDK parsed metadata source response for name or UUID {first_source}:", source_response)
            query = '[{}]'  # empty is everything
            src_tags_response = get_api_v_1_metadata_sources_srctags_source_uuid.sync(client=client, source_uuid=first_source, query=query)
            print(f"SDK parsed metadata source tags response for source UUID {first_source}:", src_tags_response)
            state_sets = get_api_v_1_metadata_sources_statesets_source_uuid.sync(client=client, source_uuid=first_source)
            ...
            if state_sets and state_sets.additional_properties:
                ...
                print(f"First state set ID: {last_state_set_id}, name: {last_state_set_name}")
                by_name = get_api_v_1_metadata_sources_statesets_name_source_uuid.sync(client=client, source_uuid=first_source, name=last_state_set_name)
                ...
                by_id = get_api_v_1_metadata_sources_statesets_id_source_uuid.sync(client=client, source_uuid=first_source, id=last_state_set_id)
